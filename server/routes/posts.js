...
router.post('/api/image', upload.single('image'), function (req, res) {
    ...
    fs.rename(file.path, path.join(__dirname, "..",'build', 'static', 'media', file.originalname), function(err) {
...
//get posts by onMain
router.get('/api/onMain', getPostByOnMain, (req, res) => {
    res.json(res.post); 
    })  
...
if(req.body.hidden != null){
    res.post.hidden = req.body.hidden;
}
if(req.body.imageURL != null){
    res.post.imageURL = req.body.imageURL;
}
if(req.body.onMain != null){
    res.post.onMain = req.body.onMain;
}
...
async function getPostByOnMain(req, res, next){
    try{
        post = await Posts.find({onMain: true },{});
        if(post == null){
            return res.status(404).json({message: "Post doesnt exist"});
        }
    }catch(err){
